...
import { Package, Plus, Edit2, Trash2, Calculator, DollarSign, PackageSearch } from 'lucide-react';
      ...
      {kits.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-12 text-center">
          <div className="bg-indigo-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4">
            <PackageSearch className="text-indigo-500" size={40} />
          </div>
          <h3 className="text-xl font-bold text-slate-800 mb-2">Nenhum kit criado</h3>
          <p className="text-slate-500 mb-6 max-w-md mx-auto">
            Crie seu primeiro combo combinando receitas e embalagens para oferecer descontos especiais aos seus clientes.
          </p>
          <button
            onClick={() => handleOpenForm()}
                      ...
                      {kit.itens.slice(0, 3).map((item, idx) => {
                        ...
                        return (
                          <li key={idx} className="text-sm text-slate-600 flex justify-between items-center bg-slate-50 p-2 rounded-lg">
                            <span className="truncate pr-2">{item.quantidade}x {nome || 'Item não encontrado'}</span>
                          </li>
                        );
                      })}
                      {kit.itens.length > 3 && (
